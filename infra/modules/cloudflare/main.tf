...
resource "cloudflare_page_rule" "static_assets" {
  ...
  actions {
    ...
    edge_cache_ttl    = 2678400 # 31 days (máximo permitido en plan Free)
...
resource "cloudflare_page_rule" "api_cache_control" {
  ...
  actions {
    cache_level       = "bypass"
    browser_cache_ttl = 7200 # mínimo para bypass
    edge_cache_ttl    = 7200 # mínimo para bypass
  }
}
...
resource "cloudflare_page_rule" "web_app_cache" {
  ...
  actions {
    ...
    edge_cache_ttl    = 7200 # 2 hours (mínimo permitido para aggressive)
    browser_cache_ttl = 120  # mínimo permitido
    browser_check     = "off"
  }
}

# WAF Rules - Comentado para plan Free
# Los Rulesets avanzados requieren plan Pro o superior
...
# Rate Limiting - Comentado para plan Free
...
# Security Headers - Comentado para plan Free
